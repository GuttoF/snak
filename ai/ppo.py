...
import random as rd # type: ignore # noqa
import numpy as np
...
class ActorPPO(nn.Module):
    def __init__(self, mid_dim, state_dim, action_dim):
        ...
        # the logarithm (log) of standard deviation (std) of action,
...
class ReplayBuffer:
    ...
    def extend_buffer(self, state, other):  # CPU array to CPU array
        ...
        if next_idx > self.max_len:
            end_idx = self.max_len - self.next_idx
            self.buf_state[self.next_idx:self.max_len] = state[:end_idx]
            self.buf_other[self.next_idx:self.max_len] = \
                other[:self.max_len - self.next_idx]
            self.if_full = True
    ...
    def sample_batch(self, batch_size):
        indices = np.random.randint(0, self.now_len - 1, size=batch_size)
...
def layer_norm(layer, std=1.0, bias_const=1e-6):
    torch.nn.init.orthogonal_(layer.weight, int(std))
